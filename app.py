...
if menu == "Registro de Destajos":
   ...
   # === RESTAURACIÓN DE BOTONES EXTERNOS ===
    b_col1, b_col2, b_col3, b_col4, b_col5 = st.columns([1.5, 1.5, 2, 2, 2])
    
    if b_col1.button("☑️ Seleccionar Todos", use_container_width=True):
        # Filtra estrictamente las filas que están en pantalla y no están bloqueadas
        indices_pendientes = df_filtrado[df_filtrado['Fecha pago'].fillna('').astype(str).str.strip() == ''].index
        st.session_state.df.loc[indices_pendientes, 'Pagar'] = True
        st.session_state.reload_trigger = True
        st.session_state.ignorar_fantasma = True # 🛡️ Escudo activado
        st.rerun()
    ...
    # 1. CONFIGURAMOS UNA ÚNICA COLUMNA PARA CHECKBOX (La original)
    gb.configure_column("Pagar", headerName="☑️ Sel", editable=True, cellRenderer='agCheckboxCellRenderer', cellEditor='agCheckboxCellEditor', cellClass='centrar-valor', headerClass='ag-center-header', width=90)
    ...
    # === CAPTURAMOS LAS ÓRDENES ===
    recargar_tabla = st.session_state.get('reload_trigger', False)
    ignorar_sincronizacion = st.session_state.get('ignorar_fantasma', False)

    # === TABLA EN TIEMPO REAL (CON LLAVE FIJA INAMOVIBLE) ===
    response = AgGrid(
        ...
        key="grid_destajos_maestro", # <-- Llave fija, tus filtros están a salvo
        reload_data=recargar_tabla,
    ...
    # Apagamos los triggers para el siguiente clic
    st.session_state.reload_trigger = False
    st.session_state.ignorar_fantasma = False
    ...
    if response['data'] is not None and not pd.DataFrame(response['data']).empty:
        ...
        # Solo leemos los clics manuales de la pantalla si NO venimos de presionar un botón masivo.
        # Esto soluciona que "Seleccionar Ninguno" no deseleccionara.
        if not ignorar_sincronizacion:
        ...
        # (A partir de aquí dejas tu código normal intacto que calcula las sumas...)
