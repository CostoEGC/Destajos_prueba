...
# Estados de memoria para la asignación masiva ("arrastre virtual")
if 'masivo_destajista' not in st.session_state: st.session_state.masivo_destajista = {}
if 'masivo_monto' not in st.session_state: st.session_state.masivo_monto = {}
...
# CLASE PARA GENERAR REPORTE PDF (PUNTO 6)
...
if menu == "Registro de Destajos":
    ...
    # BOTÓN PARA LIMPIAR TODOS LOS FILTROS (PUNTO 1)
    if col_vacio.button("🧹 Mostrar Todos (Limpiar Filtros)"):
    ...
    # NUEVO BOTÓN DE REPORTES (PUNTO 6)
    if st.button("📄 GENERAR REPORTE PDF"):
        st.info("Función de reporte lista para integrarse según tus requerimientos estables.")
...
# =========================================================================
    ...
    COORDENADAS_LOTES = {
        ...
        "37": {"x": 498, "y": 272}, "38": {"x": 532, "y": 278}, "39": {"x": 568, "279"}, "40": {"x": 603, "y": 285},
        ...
        "141": {"x": 725, "y": 999}, "142": {"x": 768, "1006}, "143": {"x": 901, "y": 1015}, "144": {"x": 893, "y": 985},
    ...
    # --- ENTORNO OSCURO VERTICAL PERSONALIZADO PARA LA TABLA (PUNTO 8) ---
    st.markdown("### 📋 Listado y Control de Estimaciones")
    
    # Herramientas de Asignación Masiva ("Arrastre Virtual" Puntos 1, 3, 4)
    with st.expander("⚡ Herramientas de Asignación Masiva (Reemplazo de arrastrar)", expanded=True):
        st.write("Selecciona aquí un valor para aplicarlo a todas las partidas vacías del lote activo simultáneamente:")
        c_masivo1, c_masivo2 = st.columns(2)
        dest_masivo = c_masivo1.selectbox("Asignar Destajista a todas las filas:", ["Seleccionar..."] + LISTA_DESTAJISTAS)
        monto_masivo = c_masivo2.number_input("Asignar Monto Fijo a todas las filas:", min_value=0.0, step=100.0, value=None)
        
        c_btn_mas1, c_btn_mas2 = st.columns(2)
        if c_btn_mas1.button("📋 Aplicar Destajista en Bloque"):
            if dest_masivo != "Seleccionar...":
                for idx in df_filtrado.index:
                    if float(st.session_state.df.at[idx, 'Pago_1']) == 0:
                        st.session_state.masivo_destajista[idx] = dest_masivo
                st.rerun()
        if c_btn_mas2.button("💰 Aplicar Monto en Bloque"):
            if monto_masivo is not None and monto_masivo > 0:
                for idx in df_filtrado.index:
                    precio = float(st.session_state.df.at[idx, 'Precio'])
                    p1 = float(st.session_state.df.at[idx, 'Pago_1'])
                    p2 = float(st.session_state.df.at[idx, 'Pago_2'])
                    por_pagar = precio - (p1 + p2)
                    if por_pagar > 0:
                        st.session_state.masivo_monto[idx] = min(monto_masivo, float(por_pagar))
                st.rerun()
    ...
    h4.markdown("<div class='texto-verde-resaltado'>Monto a pagar</div>", unsafe_allow_html=True)
    h5.markdown("")
    h6.markdown("<div class='texto-verde-resaltado'>Pago 1</div>", unsafe_allow_html=True)
    h7.markdown("<div class='texto-verde-resaltado'>Fecha 1</div>", unsafe_allow_html=True)
    h8.markdown("<div class='texto-verde-resaltado'>Pago 2</div>", unsafe_allow_html=True)
    h9.markdown("<div class='texto-verde-resaltado'>Fecha 2</div>", unsafe_allow_html=True)
    h10.markdown("<div class='texto-verde-resaltado'>Por pagar</div>", unsafe_allow_html=True)
    h11.markdown("<div class='texto-verde-resaltado'>Estado</div>", unsafe_allow_html=True)
    h12.markdown("<div class='texto-verde-resaltado'>Usuario</div>", unsafe_allow_html=True)
    st.markdown("<hr style='margin:5px 0 15px 0; border-color: #222222;'>", unsafe_allow_html=True)
    
    with st.container(height=750): # Altura ampliada verticalmente (Punto 8)
        if df_filtrado.empty:
            st.info("No hay partidas que coincidan con los filtros seleccionados.")
        else:
            for numero, (indice, fila) in enumerate(df_filtrado.iterrows(), start=1):
                ...
                # Renderizado con tipografía verde y estilo oscuro
                ...
                dest_val = fila.get('Destajista', '')
                if pago_1 > 0:
                    c3.markdown(f"<div class='texto-verde-resaltado'>{dest_val}</div>", unsafe_allow_html=True)
                    destajista_seleccionado = dest_val
                else:
                    val_def_dest = st.session_state.masivo_destajista.get(indice, "Seleccionar...")
                    idx_dest = (["Seleccionar..."] + LISTA_DESTAJISTAS).index(val_def_dest) if val_def_dest in (["Seleccionar..."] + LISTA_DESTAJISTAS) else 0
                    destajista_seleccionado = c3.selectbox("Destajista", ["Seleccionar..."] + LISTA_DESTAJISTAS, index=idx_dest, key=f"sel_{indice}", label_visibility="collapsed")
                
                if por_pagar > 0:
                    val_def_monto = st.session_state.masivo_monto.get(indice, None)
                    monto_input = c4.number_input("Monto", min_value=0.0, max_value=float(por_pagar), value=val_def_monto, step=100.0, key=f"monto_{indice}", label_visibility="collapsed")
                    if c5.button("💳", key=f"btn_{indice}", type="primary", use_container_width=True):
                        if destajista_seleccionado in ["Seleccionar...", "", None] or pd.isna(destajista_seleccionado):
                            st.error("⚠️ Debes seleccionar un destajista primero.")
                        elif monto_input is None or monto_input <= 0:
                            ...
                            st.error("⚠️ La cantidad supera el monto máximo a pagar.")
                        else:
                            es_pago_2 = (pago_1 > 0)
                            dialogo_confirmacion(indice, fila['Lote'], fila['Partida'], destajista_seleccionado, precio, monto_input, es_pago_2)
                else:
                    c4.markdown(f"<div style='text-align:center; color:#6b7280; font-size:{TAMANO_LETRA_TABLA}; font-weight:bold;'>Completado</div>", unsafe_allow_html=True)
                    c5.markdown("")
                
                c6.markdown(f"<div class='texto-verde-resaltado' style='text-align:center;'>$ {pago_1:,.2f}</div>", unsafe_allow_html=True)
                f1 = fila.get('Fecha_Pago', '-') if str(fila.get('Fecha_Pago', '')) != 'nan' else '-'
                c7.markdown(f"<div class='texto-verde-resaltado' style='text-align:center; font-size:9px;'>{f1}</div>", unsafe_allow_html=True)
                
                c8.markdown(f"<div class='texto-verde-resaltado' style='text-align:center;'>$ {pago_2:,.2f}</div>", unsafe_allow_html=True)
                f2 = fila.get('Fecha_Pago_2', '-') if str(fila.get('Fecha_Pago_2', '')) != 'nan' else '-'
                c9.markdown(f"<div class='texto-verde-resaltado' style='text-align:center; font-size:9px;'>{f2}</div>", unsafe_allow_html=True)
                
                color_deuda = "#EF4444" if por_pagar > 0 else "#10B981"
                c10.markdown(f"<div class='texto-verde-resaltado' style='text-align:center; color:{color_deuda}; font-weight:bold;'>$ {por_pagar:,.2f}</div>", unsafe_allow_html=True)

                color_barra = "#10B981" if pct_pagado == 100 else "#3B82F6"
                barra_html = f"""
                <div style="width: 100%; background-color: #222222; border-radius: 4px; height: 18px; position: relative; margin-top: 2px; border: 1px solid #333333;">
                    <div style="width: {pct_pagado}%; background-color: {color_barra}; height: 100%; border-radius: 4px;"></div>
                    <div style="position: absolute; top: 0; left: 0; width: 100%; text-align: center; font-size: 10px; color: white; font-weight: bold; line-height: 18px;">{pct_pagado}%</div>
                </div>
                """
                c11.markdown(barra_html, unsafe_allow_html=True)
                ...
                c12.markdown(f"<div class='texto-verde-resaltado' style='text-align:center; font-size:9px;'>{user_display}</div>", unsafe_allow_html=True)
                
    st.markdown("</div>", unsafe_allow_html=True)
    
    # PANEL INFERIOR PARA DISPARAR EL DIÁLOGO DEL REPORTE (PUNTO 6)
    st.markdown("<br>", unsafe_allow_html=True)
